   ...
   for space, windows in pairs(window_list) do
      for x, window_column in ipairs(windows) do
         for y, window in ipairs(window_column) do
            ...
            obj.logger.df("index_table[%d] = {space=%d, x=%d,y=%d}", id, index.space, index.x, index.y)
         end
      end
   end
end
...
function obj:tileSpace(anchor_window, column_index, space)
   ...
   if window_list == nil then
      return
   end
   if window_list[space] == nil then
      return
   end
   ...
   if column ~= nil then
      if #column == 1 then
         ...
         self:tileColumn(column, bounds, target_h, anchor_frame.w, anchor_window:id(), anchor_frame.h)
      end
   end
...
function obj:addWindow(add_window)
   ...
   if prev_focused_id and space == index_table[prev_focused_id].space then
      ...
      local prev_win = hs.window.get(prev_focused_id)
      local prev_frame = nil
      if prev_win then
         prev_frame = prev_win:frame()
...
function obj:focusWindow(direction, focused_index)
   ...
   elseif direction == Direction.UP or direction == Direction.DOWN then
      new_focused_window = window_list[focused_index.space][focused_index.x][focused_index.y + (direction / 2)]
   end
...
function obj:cycleWindowSize(direction)
   ...
   elseif direction == Direction.HEIGHT then
      ...
      focused_frame.y = math.max(work_area.y, focused_frame.y + ((focused_frame.h - new_height) / 2))
