   ...
   elseif #windows > 1 then
      ...
      if activeWindowIndex then
         if direction == "next" then
            ...
            if activeWindowIndex > #windows then
               activeWindowIndex = 1
            end
         else
            activeWindowIndex = activeWindowIndex - 1
...
local function highlightFocus()
   ...
   hl:setFillColor(hs.drawing.color.hammerspoon.osx_blue)
...
module.windowHints = function()
   ...
   -- local expose = hs.expose.new(nil, { onlyActiveApplication = false, includeOtherSpaces = false })
   -- expose:toggleShow()
...
module.persistPosition = function(win, option)
   ...
   if #frames > S_HS_CONFIG.window.historyLimit then
