...
local function Execute(cmd)
	...
	local ok, object, descriptor = hs.osascript.applescript(string.format('do shell script "%s"', cmd))
  -- local callback = nil
  -- local streamCallback = nil
  -- local task = hs.task.new(cmd, callback, streamCallback, args)
	if not ok then
		hs.alert "execute error"
		-- Log("execute error" .. cmd)
	end
	print(ok, object, descriptor)
	return object
	-- return Trim(output)
end
