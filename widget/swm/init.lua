...
local getCurrentSpacesIds = function()
   ...
   return hs.spaces.spacesForScreen()
end
...
M.getSpaceId = function(win)
   ...
   return spaceId or hs.spaces.focusedSpace()
end
...
local getSpaceIndex = function(win)
   ...
   local spaceIds = hs.spaces.spacesForScreen(uuid) --spaces.layout()[uuid]
...
local SKIP_BUNDLES = {
   ...
   ["com.apple.ViewBridgeAuxiliary"] = true,
   ["com.apple.AuthenticationServicesCore.AuthenticationServicesAgent"] = true,
   ...
   ["com.mac.utility.media.hub"] = true,
   ...
   ["dev.kdrag0n.MacVirt.vmgr"] = true,
   ["com.apple.TextInputUI.xpc.CursorUIViewService"] = true,
   -- ["N/A"] = true,
}
...
local getAllWindows = function()
   ...
   for _, app in ipairs(hs.application.runningApplications()) do
      ...
      if app:kind() >= 0 then
         local name = app:name()
         --just for safety; universalaccessd has no bundleid (but it's kind()==-1 anyway)
         local bid = app:bundleID() or "N/A"
...
local getAllWindowsUsingSpaces = function()
   ...
   hs.fnutils.each(spacesIds, function(spaceId)
      local windows = hs.spaces.windowsForSpace(spaceId) -- spaces.allWindowsForSpace(spaceId)
...
local getCurrentSpacesByScreen = function()
   ...
   local currentSpaces = hs.spaces.spacesForScreen()
      ...
      for screenIdx, spaceIds in pairs(cache.spaces) do
         for spaceIdx, spaceWindows in ipairs(spaceIds) do
            for winIdx, _win in ipairs(spaceWindows) do
               if _win == wId then
                  return _win, spaceIdx, winIdx, screenIdx
               end
            end
         end
      end
...
M.getLayout = function(spaceId)
   ...
   local layout = hs.spaces.allSpaces() --spaces.layout()
...
M.throwToScreenUsingSpaces = function(win, direction)
   ...
   hs.spaces.moveWindowToSpace(win, throwToSpaceId)
...
M.throwToSpaceIdx = function(win, screenIdx, spaceIdx)
   ...
   local spaceId = hs.spaces.spacesForScreen(targetScreen:getUUID())[spaceIdx]
   ...
   if M.isFloating(win) then
      ...
      if targetScreen:name() ~= win:screen():name() then
         ...
         win:setTopLeft(newX, newY)
         hs.spaces.goToSpace(spaceId, false)
      end
...
M.tiling = function()
   ...
   hs.fnutils.each(currentSpaces, function(spaceId)
      ...
      local spaceIdx = getSpaceIdx(spaceId, screen:getUUID())
      ...
      local checkDuplicate = function(windows)
         for i = #windows, 1, -1 do
            ...
            local existsOnScreen = hs.fnutils.find(tilingWindows, function(win)
               return win == windows[i] and win:spaces()[1] == spaceId
            end)
            ...
            for j = 1, #windows do
               if windows[i] == windows[j] and i ~= j then
                  duplicateIdx = j
               end
            end
   ...
   hs.fnutils.each(currentSpaces, function(spaceId)
      ...
      local screenWindows = hs.fnutils.filter(spaceWindows, function(win)
         return win:title() ~= "" and win:screen():id() == screen:id()
      end)
      ...
      if not layoutName or not layouts[layoutName] then
         ...
         for index, window in pairs(screenWindows) do
            ...
            -- log.d(
            --   hs.inspect {
            --     id = hs.inspect(window:id()),
            --     role = hs.inspect(window:role()),
            --     title = hs.inspect(window:title()),
            --     name = hs.inspect(window:application():name()),
            --   }
            -- )
         end
      end
